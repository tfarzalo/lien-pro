  ...
  <section ref={ref as React.RefObject<HTMLElement>} className="max-w-screen-2xl mx-auto px-6 py-8">
    ...
    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 lg:gap-8">
      {cards.map((card, index) => (
        <div
          key={card.name}
          className={`group rounded-3xl border bg-white dark:bg-slate-900 p-6 lg:p-8 flex flex-col gap-6 transition-all duration-700 shadow-sm hover:shadow-2xl hover:-translate-y-2 ${
            card.featured ? 'border-brand-300 dark:border-brand-700 bg-gradient-to-br from-brand-50/80 to-white dark:from-brand-950/50 dark:to-slate-900 relative ring-2 ring-brand-200 dark:ring-brand-800' : 'border-slate-200 dark:border-slate-800 hover:border-brand-200 dark:hover:border-brand-800'
        ...
        >
          ...
          <div className="mt-auto">
            ...
            <a
              href={card.href}
              className={`w-full inline-flex justify-center items-center rounded-2xl px-5 py-3.5 font-semibold transition-all duration-300 ${
                card.featured
